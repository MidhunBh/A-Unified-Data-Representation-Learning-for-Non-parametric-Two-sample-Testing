...
x_in = 2  # HDGM d=2 historical reconstruction
...
N_TRAIL = 100  # full confirmation matching author outer-trial count
N_TEST = 100  # number of test sets
N_TEST_F = 100.0  # number of test sets (float)0
N_PER = 100  # permutation times

n_list = [500, 750, 1000, 1250]
#n_list = [250, 375, 500]
# n_list = [7.5, 10, 12.5]
...
for n in n_list:
    ...
    for kk in range(N_TRAIL):
        ...
        # epoch = 2000  # current frozen implementation; disabled for historical reconstruction

        encoder = train_autoencoder(S_encoder, epoch, x_in, H, x_out, batch_size, device, dtype)
        # Historical 55570272 path: encoder remains trainable in Phase 2
        ...
        # Historical d=2 path used the raw encoder directly
...
with open('result/author55570272_rlc2st_d2_remaining4.pkl', 'wb') as file:
    pickle.dump(c2st_semi_baseline_result_t2, file)

import json, time

meta = {
    "method": "RL-C2ST (AE), historical author 55570272 reconstruction",
    "dataset": "HDGM-D",
    "level": "hard",
    "metric": "test power",
    "panel": "Fig 3b / Fig 4a diagnostic",
    "source_slurm": "HDGM_semi_baseline55570272.out",
    "d": x_in,
    "internal_n": n_list,
    "paper_N": [8*n for n in n_list],
    "N_TRAIL": N_TRAIL,
    "N_TEST": N_TEST,
    "N_PER": N_PER,
    "classifier_epochs": N_EPOCH,
    "ae_epoch_formula": "int(400 * 2000 / (n_train + n_test))",
    "ae_epochs": [
        int(400 * 2000 / (n + n))
        for n in n_list
    ],
    ...
    "ae_lr": 0.002,
    "c2st_lr": 0.005,
    "encoder_frozen_phase2": False,
    "phase2_model": "raw encoder passed directly to C2ST_NN_fit",
    "scheduler": False,
    "author_logged_statistic": "C2ST-L",
   "author_target_L": {
    "500": 1.0,
    "750": 1.0,
    "1000": 1.0,
    ...
    "note": (
    "100-trial extension of the historical d=2 reconstruction associated "
    "with author SLURM job 55570272, covering the remaining four Figure 3 "
    "sample sizes. Both C2ST-S and C2ST-L are evaluated from the same fitted "
    "model to investigate the paper/log statistic-label ambiguity."
    ),
...
with open("result/author55570272_rlc2st_d2_remaining4.json", "w") as f:
    json.dump(meta, f, indent=2)
print("Saved historical 55570272 remaining-four-point run:")
print("  result/author55570272_rlc2st_d2_remaining4.pkl")
print("  result/author55570272_rlc2st_d2_remaining4.json")
