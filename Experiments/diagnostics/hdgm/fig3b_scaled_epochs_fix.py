...
for n in [125, 250]:
    epoch = int(400 * 2000 / (n + n))  # the authors' own commented-out scaling formula
    print(f"\n=== n={n} (N={8*n}), scaled AE epochs={epoch} ===")
    summary_s, summary_l = [], []
    for kk in tqdm.trange(N_TRAIL, desc=f"n={n}"):
        ...
        encoder = train_autoencoder(S_encoder, epoch, x_in, H, x_out, 512, device, torch.float, lr=0.002)
        ...
        model_C2ST_L, w, b = C2ST_NN_fit(S, y, x_in, H, x_out, 1000, 1024, device, torch.float, base_model, lr_c2st=0.002)
    ...
    print(f"(compare: authors' winning run -- n125:0.32/? n250:0.88/?)")
