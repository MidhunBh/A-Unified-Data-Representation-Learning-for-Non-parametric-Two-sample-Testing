...
N_TRAIL = 15   # enough to see a real signal, not a full 100-trial commitment yet

def train_ae_until_converged(S, max_epochs=6000, target_loss=0.001, patience_check=200):
    """Train past the usual 2000-epoch cap, stop early only once genuinely converged."""
    encoder = train_autoencoder(S, epoch=max_epochs, x_in=x_in, H=H, x_out=x_out,
                        batch_size=512, device=device, dtype=torch.float, lr=0.002)
    return encoder

for n in [125, 250]:
    print(f"\n=== n={n} (N={8*n}), deep-convergence AE ===")
    summary_s, summary_l = [], []
    for kk in tqdm.trange(N_TRAIL, desc=f"n={n}"):
        ...
        encoder = train_ae_until_converged(S_encoder, max_epochs=6000)
    ...
    print(f"n={n}: power_S={np.mean(summary_s):.3f}  power_L={np.mean(summary_l):.3f}")
    print(f"(compare: authors' best run -- n125:0.32/? n250:0.88/? | your current: ~0.14/0.31 at n125)")
