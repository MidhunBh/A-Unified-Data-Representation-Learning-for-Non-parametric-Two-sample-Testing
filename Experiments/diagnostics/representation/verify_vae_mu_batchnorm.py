...
class VAE_Img(nn.Module):
    def __init__(self, channels, img_size, z_size):
        ...
        self.fc_mu = nn.Sequential(nn.Linear(z_size, z_size), nn.BatchNorm1d(z_size))  # the fix
...
print("Conv VAE, BatchNorm on mu (the actual runaway term)...")
for step in range(N_STEPS):
    ...
    if (step + 1) % 50 == 0:
        print(f"step {step+1}: beta={beta:.2f} recon={recon_loss.item():.4f} kl={kl.item():.4f} mean(mu^2)={mu.pow(2).mean().item():.4f}")
