...
N_STEPS, WARMUP, LAMBDA_TARGET = 400, 200, 10.0
print("WAE with lambda_mmd annealing (0 warmup, then ramp to 10)...")
for step in range(N_STEPS):
    lam = 0.0 if step < WARMUP else LAMBDA_TARGET * min(1.0, (step - WARMUP) / WARMUP)
    recon, z = model(dummy)
