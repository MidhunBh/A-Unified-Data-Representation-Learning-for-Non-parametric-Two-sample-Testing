...
alpha = 0.05
n = 250  # N=2000, matches existing lambda=1 100-trial anchor point
N_TRAIL = 15
N_TEST = 30
N_PER = 100
AE_EPOCHS = 2000
...
print("Bracketing around lambda=0.1 (best so far: S=0.200 L=0.667). lambda=0.01 already reversed (S=0.200 L=0.467)\n")

for lambda_target in [0.3, 0.05]:
    print(f"=== lambda_target={lambda_target}, N=2000, {N_TRAIL} trials ===")
    summary_s, summary_l = [], []
    ...
    print(f"lambda={lambda_target}: power_S={np.mean(summary_s):.3f}  power_L={np.mean(summary_l):.3f}\n")
