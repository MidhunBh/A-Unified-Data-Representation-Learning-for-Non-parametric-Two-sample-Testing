...
# ============================================================
# PILOT: transferred WAE z=10, lambda=0.01, frozen encoder + nonlinear 30-D C2ST head
#
...
#
# Corrected HDGM WAE reconstruction retains:
#   - representation-learning epoch schedule
#   - batch size
#   - representation-learning LR
#   - frozen WAE encoder passed into a nonlinear C2ST head
#   - C2ST LR
...
parser.add_argument(
    ...
    default=0.01,
    help=(
        "WAE MMD penalty weight. Default 0.01 is the locked "
        "Phase-1-selected transfer value from the d=2 reconstruction; "
...
# Transferred validated WAE settings
WAE_BATCH = 512
C2ST_BATCH = 1024
WAE_LR = 0.002
...
output_stem = (
    f"rl_c2st_d10_z10_AEequiv_control_{args.shard_id}"
)
...
def train_wae(
    ...
    for epoch in range(epochs):
        ...
        for (x_batch,) in loader:
            ...
            if lambda_mmd == 0.0:
                # Exact ordinary-autoencoder control.
                # Do not consume RNG by drawing a latent prior.
                mmd_loss = recon_loss.new_tensor(0.0)
                loss = recon_loss
            else:
                # WAE-MMD branch.
...
# ============================================================
# Update-count-matched representation epoch schedule
#
...
def representation_epochs(n_train, n_test):
    ...
    # Historical d=10 AE reproduction used a fixed
    # 2000 representation-learning epochs.
    return 2000
...
def build_payload(records):
    ...
    return {
        ...
        "representation": {
            ...
            "lambda_provenance": (
                "lambda=0.01 was selected using Phase-1 diagnostics and transferred to z=10 "
                "without using d=10 downstream test power. "
                "Tian's exact WAE lambda is not available."
            ),
            "reconstruction_loss": "PyTorch MSELoss",
            "learning_rate": WAE_LR,
            "wae_batch_size": WAE_BATCH,
        "c2st_batch_size": C2ST_BATCH,
            "epoch_schedule": (
        ...
        "phase2": {
            "model": (
                "frozen z=10 WAE encoder followed by trainable 10->30->30->30 nonlinear C2ST feature head"
            ),
            "encoder_frozen": True,
            "epochs": N_EPOCH,
            "wae_batch_size": WAE_BATCH,
        ...
        "note": (
            ...
            "The corrected HDGM WAE training recipe "
            ...
            "phase 2 through a frozen z=10 encoder and trainable nonlinear 30-D head, with C2ST LR=0.005. "
...
# ============================================================
# DIAGNOSTIC: nonlinear C2ST probe on frozen z=2 WAE
#
# Previous frozen pilot used:
#
#       WAE z=2 -> linear 2-class weights
#
# which is substantially weaker than the historical Tian
# d=2 path whose encoder exposed 30 features to C2ST.
#
# Here:
#
#       frozen WAE z=10
#             |
#             v
#       trainable 10 -> 30 -> 30 -> 30 feature head
#             |
#             v
#       ordinary C2ST final weights
#
# This tests whether the fixed z=2 WAE representation still
# contains the P/Q discrepancy but in a nonlinear form.
# ============================================================

C2ST_FEATURE_DIM = 30


class FrozenZ10MLPHead(nn.Module):
    ...
    def __init__(self, encoder):
        ...
        # WAE representation itself must remain fixed.
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(H, C2ST_FEATURE_DIM, bias=True),
        )
...
for n in n_list:
    ...
    for kk in progress:
        ...
        # Keep Phase-1 WAE representation fixed, but restore
        # nonlinear classifier capacity above the z=2 bottleneck.
        # ----------------------------------------------------
        # MATCHED PHASE-2 INITIALIZATION CONTROL
        #
        # train_wae() consumes a large amount of torch RNG state
        # during 1600 representation-learning epochs.
        #
        # Without resetting here, the nonlinear head following
        # the WAE starts from different random weights than the
        # RAW matched-head control.
        #
        # Resetting immediately before head construction makes
        # the trainable 10 -> 30 -> 30 -> 30 head numerically
        # initialization-matched to the RAW control.
        #
        # The ONLY intended difference is therefore:
        #
        #     RAW: x -> head
        #
        #     WAE: x -> frozen WAE(z=10) -> same head
        # ----------------------------------------------------
        ...
        phase2_model = FrozenZ10MLPHead(encoder).to(device, dtype)
