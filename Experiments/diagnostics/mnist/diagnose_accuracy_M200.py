...
print(f"=== Raw classification accuracy diagnostic, M={n}, {N_TRAIL} trials ===")
accuracies = []
for kk in tqdm.trange(N_TRAIL, desc=f"M={n} accuracy check"):
    n_train = n
    n_test = n
    ...
    y_test = torch.concat((torch.zeros(n_test), torch.ones(n_test))).to(device, dtype).long()
    ...
    with torch.no_grad():
        preds = c_model(S_test).argmax(dim=1)
        acc = (preds == y_test).float().mean().item()
    accuracies.append(acc)
    print(f"  trial {kk}: held-out accuracy = {acc:.3f}")

print(f"\nMean held-out accuracy across {N_TRAIL} trials: {np.mean(accuracies):.4f}")
print(f"Min: {min(accuracies):.4f}  Max: {max(accuracies):.4f}")
print("(0.5 = chance/indistinguishable, 1.0 = perfectly separable)")
