...
for n in n_list:
    ...
    for kk in tqdm.trange(N_TRAIL):
        ...
        model_mmd, sigma, sigma0, ep_val = MMD_D_fit(
            IR_tr, ae_latent, 30, 30, N_EPOCH, device, dtype, lr_mmd=0.000005)
        ...
        print(f"Test Power of RL-MMD-D at paper M={2*n} (internal n={n}): ", H_MMD.sum() / N_TEST_F)
        summary_n.append(H_MMD.sum() / N_TEST_F)

    summary.append(summary_n)
    print(f"Average Test Power of RL-MMD-D at paper M={2*n} (internal n={n}): ", np.mean(summary_n))

with open("result/diagnose_rlmmdd_mnist_M200_lr5e6_10trials.pkl", "wb") as f:
    pickle.dump(summary, f)

import json, subprocess, time
meta = {
    "commit": "diagnostic-no-git-cli",
    "method"  : "RL-MMD-D",
    "dataset" : "MNIST vs Fake MNIST",
    "metric"  : "test power",
    "panel"   : "Table 3",
    "internal_n": n_list,
    "paper_M": [2*x for x in n_list],
    "N_TRAIL" : N_TRAIL,
    "N_EPOCH" : N_EPOCH,
    "lr_mmd": 0.000005,
    "note": "Original MNIST RL-MMD-D latent-head template; only change is lr_mmd=5e-6, transferred from independently established HDGM RL-MMD-D correction.",
    "result"  : [float(np.mean(r)) for r in summary],
    "ts"      : time.strftime("%Y-%m-%d %H:%M"),
}
with open("result/diagnose_rlmmdd_mnist_M200_lr5e6_10trials.json", "w") as f:
    json.dump(meta, f, indent=2)
print("logged to result/diagnose_rlmmdd_mnist_M200_lr5e6_10trials.json")