...
OUTPUT = (
    ...
    "reconstruction_wrapper_frozen_vs_joint_10trials.json"
)
...
for kk in range(
    ...
    print(
        ...
        f"frozen={frozen['conditional_rejection']:.2f} "
        f"(run={mean_frozen:.3f}, "
        f"J={frozen['final_J']:.2f}) | "
        f"joint={joint['conditional_rejection']:.2f} "
        f"(run={mean_joint:.3f}, "
        f"J={joint['final_J']:.2f})",
        flush=True,
    )
...
payload = {
    "method": (
        "RL-MMD-D reconstruction-wrapper diagnostic"
    ),
...
print(
    "FINAL RECONSTRUCTION-WRAPPER DIAGNOSTIC"
)
