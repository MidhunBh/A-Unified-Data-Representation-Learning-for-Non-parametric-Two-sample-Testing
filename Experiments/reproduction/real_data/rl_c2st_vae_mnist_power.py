...
class VAE_Img(nn.Module):
    def __init__(self, channels, img_size, z_size):
        ...
        self.fc_mu = nn.Sequential(nn.Linear(z_size, z_size), nn.BatchNorm1d(z_size))
...
def train_vae_img(S, device, dtype):
    model = VAE_Img(channels, img_size, z_size).to(device, dtype)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.002)
    loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(S), batch_size=200, shuffle=True)
    for epoch in range(AE_EPOCHS):
        beta = 0.0 if epoch < WARMUP_EPOCHS else min(1.0, (epoch - WARMUP_EPOCHS) / WARMUP_EPOCHS)
        for (x,) in loader:
    ...
    model.eval()  # BatchNorm needs running stats for Phase 2/3
    return model
...
for n in n_list:
    ...
    for kk in tqdm.trange(N_TRAIL, desc=f"M={n}"):
        ...
        vae = train_vae_img(S_pool, device, dtype)
        for p in vae.parameters(): p.requires_grad = False

        with torch.no_grad():
            IR_s1_tr, _ = vae.encode(s1_tr.to(device, dtype))
            IR_s1_te, _ = vae.encode(s1_te.to(device, dtype))
            IR_s2_tr, _ = vae.encode(s2_tr.to(device, dtype))
            IR_s2_te, _ = vae.encode(s2_te.to(device, dtype))
    ...
    vae_result.append({"M": n, "RL-C2ST-S": power_s, "RL-C2ST-L": power_l})
    print(f"M={n}: RL-S(VAE)={power_s:.3f}  RL-L(VAE)={power_l:.3f}", flush=True)

    with open("result/rl_c2st_vae_mnist_power.pkl", "wb") as f:
        pickle.dump(vae_result, f)
    try:
        ...
        commit = "unknown"
    meta = {
        "commit": commit, "method": "RL-C2ST (VAE, conv)", "dataset": "MNIST vs Fake MNIST",
        "metric": "test power", "panel": "Fig 3a - VAE bar", "M_planned": n_list,
        "M_completed": [r["M"] for r in vae_result], "N_TRAIL": N_TRAIL,
        "note": "Conv VAE (Encoder_Img/Decoder_Img backbone). Two failed diagnoses before this worked: logvar clamping (wrong variable, logvar was never exploding) -- root cause was unconstrained mu drifting unbounded during pure reconstruction training (mean(mu^2) 2.94->65.24 with zero KL gradient influence, confirmed by isolating recon-only loss). Fixed with BatchNorm1d on mu's output. Frozen after Phase1 (matching HDGM convention, not MNIST's joint fine-tuning), N_TRAIL=10 matching validated MNIST convention.",
        "result": vae_result, "ts": time.strftime("%Y-%m-%d %H:%M"),
    }
    with open("result/rl_c2st_vae_mnist_power.json", "w") as f:
        json.dump(meta, f, indent=2)

print("=== DONE ===")
for r in vae_result: print(r)
