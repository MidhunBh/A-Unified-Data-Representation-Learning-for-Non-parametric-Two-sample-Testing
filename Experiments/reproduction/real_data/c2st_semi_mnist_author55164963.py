...
with open("result/c2st_semi_mnist_author55164963_reconstruction.pkl", "wb") as f:
    pickle.dump({"internal_n": n_list, "paper_M": [2*x for x in n_list], "raw_outer_run_power": summary}, f)
...
# Reproducibility record for reconstruction of author SLURM job 55164963
import json
import platform
import socket
import subprocess
...
raw_power = [[float(x) for x in values] for values in summary]
author_power = [0.4909, 0.9846, 1.0, 1.0, 1.0]

meta = {
    "schema_version": 1,
    "experiment_id": "mnist_author55164963_reconstruction",
    "purpose": "Reconstruct the C2ST-D execution path recorded in author SLURM log mnist_semi55164963.out",
    "script": os.path.basename(__file__),
    "git_commit": _get_git_commit(),
    "timestamp_utc": datetime.now(timezone.utc).isoformat(),

    "dataset": "MNIST vs Fake MNIST",
    "reference_author_log": "mnist_semi55164963.out",

    "method": {
        "logged_name": "C2ST-D",
        "test_function": "TST_MMD_l",
        "kernel": "poly",
        "representation": "first module of discriminator.adv_layer",
        "representation_dim": 100,
        "autoencoder_epochs": 1000,
        "autoencoder_latent_dim": 100,
        "autoencoder_lr": 0.002,
        "classifier_lr": 0.0004,
        "classifier_training_epochs": "2 * internal_n"
    },

    "sampling": {
        "internal_n": [int(x) for x in n_list],
        "paper_M": [int(2*x) for x in n_list],
        "paper_sample_mapping": "paper M = 2 * internal_n",
        "actual_outer_repetitions": 100,
        "N_TRAIL_constant_in_source": int(N_TRAIL),
        "test_evaluations_per_outer_run": int(N_TEST),
        "permutations_per_test": int(N_PER),
        "alpha": float(alpha)
    },
    ...
    "author_reference_power": {
        str(int(n)): float(p)
        for n, p in zip(n_list, author_power)
    },
    ...
    "results": [
        {
            ...
            "std_power": float(np.std(values, ddof=1)),
            "min_power": float(np.min(values)),
            "max_power": float(np.max(values)),
            "n_outer_runs": len(values),
            "author_log_power": float(ref),
            "delta_vs_author": float(np.mean(values) - ref),
            "raw_outer_run_power": values
        }
        for n, values, ref in zip(n_list, raw_power, author_power)
    ]
}

with open(
    "result/c2st_semi_mnist_author55164963_reconstruction.json", "w"
) as f:
    json.dump(meta, f, indent=2)

print("Saved reconstruction results:")
print("  result/c2st_semi_mnist_author55164963_reconstruction.pkl")
print("  result/c2st_semi_mnist_author55164963_reconstruction.json")
