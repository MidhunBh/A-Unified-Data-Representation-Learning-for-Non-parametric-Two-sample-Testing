...
# ============================================================
# FULL GRID: locked WAE z=2, lambda=0.01, frozen encoder + nonlinear 30-D C2ST head
#
...
def build_payload(records):
    ...
    return {
        ...
        "representation": {
            ...
            "lambda_provenance": (
                "For this pilot lambda=0.01 was selected from the z=2 "
                "Phase-1 reconstruction/prior-MMD diagnostic before downstream power. "
        ...
        "phase2": {
            "model": (
                "frozen z=2 WAE encoder followed by trainable 2->30->30->30 nonlinear C2ST feature head"
            ),
            "encoder_frozen": True,
            "epochs": N_EPOCH,
            "batch_size": batch_size,
        ...
        "note": (
            ...
            "phase 2 through a frozen z=2 encoder and trainable nonlinear 30-D head, with C2ST LR=0.005. "
...
#             |
#             v
#       trainable 2 -> 30 -> 30 -> 30 feature head
#             |
#             v
...
class FrozenZ2MLPHead(nn.Module):
...
for n in n_list:
    ...
    for kk in progress:
        ...
        # the trainable 2 -> 30 -> 30 -> 30 head numerically
        ...
        #     RAW: x -> head
        #
        #     WAE: x -> frozen WAE(z=2) -> same head
        # ----------------------------------------------------
        ...
        phase2_model = FrozenZ2MLPHead(encoder).to(device, dtype)
