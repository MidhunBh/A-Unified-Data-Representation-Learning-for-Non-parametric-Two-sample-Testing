...
AE_EPOCHS = 2000
WARMUP_EPOCHS = 1000
LAMBDA_TARGET = 1.0   # corrected from 10.0 -- see note
N_EPOCH = 1000
N_TRAIL = 100
N_TEST = 100
N_TEST_F = 100.0
...
def train_wae(S, device, dtype, x_in=x_in, H=H, x_out=x_out):
    model = WAE(x_in, H, x_out).to(device, dtype)
    ...
    for epoch in range(AE_EPOCHS):
        if epoch < WARMUP_EPOCHS:
            ...
            lam = LAMBDA_TARGET * min(1.0, (epoch - WARMUP_EPOCHS) / WARMUP_EPOCHS)
        for (x,) in loader:
            recon, z = model(x)
...
for n in n_list:
    ...
    with open("result/rl_c2st_wae_lambda1_HDGM_d2_power.pkl", "wb") as f:
    ...
    meta = {
        "commit": commit, "method": "RL-C2ST (WAE, lambda_mmd=1)", "dataset": "HDGM-D", "level": "hard",
        "metric": "test power", "panel": "Fig 3b - 4th bar (retry)", "d": x_in,
        ...
        "note": "Retry of rl_c2st_wae_HDGM_d2_power.py (lambda_mmd=10), which underperformed C2ST at N=1000 and N=2000 (0.020/0.050 and 0.060/0.030 vs C2ST's 0.150/0.300 and 0.470/0.830), and got WORSE from N=1000 to N=2000 on the L statistic -- genuine signal erosion from over-regularization, not a warmup issue (reconstruction was already low, 0.0018, in the broken run). Fixed by lowering LAMBDA_TARGET from 10.0 to 1.0, confirmed via diagnostic before this run.",
        "result": wae_result, "ts": time.strftime("%Y-%m-%d %H:%M"),
    }
    with open("result/rl_c2st_wae_lambda1_HDGM_d2_power.json", "w") as f:
