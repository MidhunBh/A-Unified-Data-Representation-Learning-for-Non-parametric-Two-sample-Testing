...
wae_result = []
already_done_N = set()
if os.path.exists("result/rl_c2st_wae_bn_HDGM_d10_power.json"):
    with open("result/rl_c2st_wae_bn_HDGM_d10_power.json") as f:
        prior = json.load(f)
    wae_result = prior["result"]
    already_done_N = set(prior["N_completed"])
    print(f"Resuming -- already completed N values: {sorted(already_done_N)}")

for n in n_list:
    if 8*n in already_done_N:
        print(f"skipping N={8*n}, already done")
        continue
    n_train = n_test = n
    summary_s, summary_l = [], []
    ...
    with open("result/rl_c2st_wae_bn_HDGM_d10_power.pkl", "wb") as f:
