...
for n in n_list:
    ...
    for kk in range(N_TRAIL):
        ...
        print(f"Type-I error of RL-C2ST-S at n = {n_train}: ", H_C2ST_S.sum() / N_TEST_F)
        print(f"Type-I error of RL-C2ST-L at n = {n_train}: ", H_C2ST_L.sum() / N_TEST_F)
        ...
        # summary.append(h1)
        print("\n\n=====================================================")
        print("Average Type-I error of RL-C2ST-S: ", np.mean(summary_s))
        print("Average Type-I error of RL-C2ST-L: ", np.mean(summary_l))
        print("=====================================================\n\n")

    c2st_semi_baseline_result_t1.append((np.mean(summary_s), np.mean(summary_l)))

print(c2st_semi_baseline_result_t1)
        
with open('result/c2st_semi_HDGM_d2_type1.pkl', 'wb') as file:
    pickle.dump(c2st_semi_baseline_result_t1, file)
...
meta = {
    ...
    "result"  : [
        ...
        for n, (s, l) in zip(n_list, c2st_semi_baseline_result_t1)
    ],
    "ts"      : time.strftime("%Y-%m-%d %H:%M"),
}
with open("result/c2st_semi_HDGM_d2_type1.json", "w") as f:
    json.dump(meta, f, indent=2)
print("logged to result/c2st_semi_HDGM_d2_type1.json")
