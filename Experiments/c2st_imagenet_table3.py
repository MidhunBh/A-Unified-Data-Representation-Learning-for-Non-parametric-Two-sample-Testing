...
data_real_all = torch.from_numpy(
    pickle.load(open("/system/user/publicwork/mbhaskar/imagenet_reproduction/processed/val_data.pkl", "rb"))[:10000])

data_fake_all = torch.from_numpy(
    pickle.load(open("/system/user/publicwork/mbhaskar/imagenet_reproduction/processed/images_data.pkl", "rb")))

n_list = [100, 200, 300, 400, 500]

RESULT_DIR = Path("/system/user/publicwork/mbhaskar/imagenet_reproduction/results")
RESULT_DIR.mkdir(parents=True, exist_ok=True)

results = {
    "paper_M": [200, 400, 600, 800, 1000],
    "author_n": n_list,
    "C2ST": {},
    "C2ST_L": {},
    "config": {
        ...
        "test_repetitions": N_TEST,
        "permutations": N_PER,
        "alpha": alpha,
        "batch_size": batch_size,
        "learning_rate": 0.0002,
        "paper_M_equals_2n": True,
    },
}
...
for n in n_list:
    ...
    results["C2ST"][str(paper_M)] = {
        "author_n": int(n),
        "outer_power": [float(x) for x in summary_s],
        "mean_power": float(np.mean(summary_s)),
    }

    results["C2ST_L"][str(paper_M)] = {
        "author_n": int(n),
        "outer_power": [float(x) for x in summary_l],
        "mean_power": float(np.mean(summary_l)),
    }

    with open(RESULT_DIR / "c2st_imagenet_table3_power.pkl", "wb") as f:
        pickle.dump(results, f)

    with open(RESULT_DIR / "c2st_imagenet_table3_power.json", "w") as f:
        json.dump(results, f, indent=2)

    print()
    print("============================================================")
    print(f"COMPLETED paper M={paper_M} / author n={n}")
    print(f"C2ST   mean power: {np.mean(summary_s):.4f}")
    print(f"C2ST-L mean power: {np.mean(summary_l):.4f}")
    print("Results checkpoint saved.")
    print("============================================================")
